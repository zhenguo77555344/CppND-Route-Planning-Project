...
void RouteModel::Node::find_neighbors()
{
	...
	for (auto& road : parent_model->node_to_road[this->index_])
	{
		...
		if (new_neighbor)
		{
			...
			std::cout<<"new_neighbor is:"<<new_neighbor->x<<","<<new_neighbor->y<<std::endl;
		}
	}
}

RouteModel::Node::Node(int idx, RouteModel* search_model, Model::Node node) : Model::Node(node),parent_model(search_model), index_(idx){}
